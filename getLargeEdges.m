function lines = getLargeEdges(grayIm, minLen)
...
inds = find(sum(double(lines(:,1:2)>10),2) & sum(double(lines(:,1:2)<width-10),2) & ...
sum(double(lines(:,3:4)>10),2) & sum(double(lines(:,3:4)<height-10),2));
lines = lines(inds,:);
